...
def process_video(video_path, analyzer, validator, classifier):
    """
    ...
    try:
        # Step 1: Analyze video with Twelve Labs
        analysis_results = analyzer.upload_and_analyze_video(video_path)
        
        # Step 2: Validate against campaign criteria
        validation_result = validator.validate_video(analysis_results["analysis_results"])
        
        # Step 3: If valid, classify into a Milk Mob
        mob_assignment = None
        similar_videos = []
        
        if validation_result["is_valid"]:
            mob_assignment = classifier.classify_video(analysis_results["analysis_results"])
